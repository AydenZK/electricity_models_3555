...
filtered_all_data_eda <- all_data %>% 
  filter(AreaTypeCode == "CTY") %>% 
  inner_join(countries, by="AreaCode") %>% 
  mutate(
    AreaName = AreaName.y, 
    ...
    dayofweek = ifelse(weekdays(Date) %in% c("Saturday", "Sunday"), 1, 0)) %>% 
  select(Date, DateTime, Year, Month, Day, Hours, dayofweek, Minutes, AreaName, ResolutionCode, TotalLoadValue) %>% 
  group_by(DateTime, Date, Year, Month, Day, Hours, AreaName, dayofweek) %>% 
  summarise(TotalLoadValue = sum(TotalLoadValue))

write.csv(filtered_all_data, "../data/all_data_hourly_eda.csv")
print("Data saved: data/all_data_hourly_eda.csv")
